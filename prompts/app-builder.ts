...
5. Use realistic sample data so the result is compelling immediately.
6. Design for desktop and mobile with strong typography, spacing, contrast, and accessible focus states.
7. Prefer CSS Grid and Flexbox. Inline SVG is allowed; Unicode emoji is allowed.
8. Do not inject unsanitized user input with innerHTML. Prefer textContent and safe DOM construction.
9. When current code is supplied, preserve its useful behavior and change only what the user requested.
10. changes must summarize visible work. suggestions must contain 2–4 relevant, actionable next steps.
11. Escape JSON correctly. Do not wrap the response in Markdown fences and do not add prose.`;
